...
fun formatNights( nights: List<SleepNight>, resources: Resources): Spanned {
    ...
    sb.apply {
        ...
        nights.forEach { night ->
            ...
            append("\t${convertLongToDateString(night.startTimeMilli)}<br>")
            if (night.endTimeMilli != night.startTimeMilli) {
                append(resources.getString(R.string.end_time))
                append("\t${convertLongToDateString(night.endTimeMilli)}<br>")
                append(resources.getString(R.string.quality))
                append("\t${convertNumericQualityToString(night.sleepQuality, resources)}<br>")
                append(resources.getString(R.string.hours_slept))
                append("\t ${night.endTimeMilli.minus(night.startTimeMilli) / 1000 / 60 / 60}:")
                append("${night.endTimeMilli.minus(night.startTimeMilli) / 1000 / 60}:")
                append("${night.endTimeMilli.minus(night.startTimeMilli) / 1000}<br><br>")
            }
        }
